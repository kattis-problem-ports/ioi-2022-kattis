# Local copy of testdata_tools/gen.sh, patched as described in
# https://github.com/Matistjati/Multiplicative_accept_score_Kattis so that the
# output validator's accept_with_score(x) multiplies the group score by x.
# The only changes are the added "grading: custom" and "subtask_score=" grader
# flags in group() and _setup_dirs().
...
accept_score: $score
range: 0 $score
grader_flags: min subtask_score=$score
grading: custom" > "$CURGROUP_DIR/testdata.yaml"
