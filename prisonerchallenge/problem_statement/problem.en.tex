...
\begin{quote}
\textit{Note:} this problem is only solvable in C++.
\end{quote}
...
\texttt{prison.h} and the sample input. Compile it with your solution and run it on an input file to
try your solution locally. It reads a single line containing $N$, and prints
\begin{itemize}
  \item line $1$: $x$
  \item line $2 + i$ ($0 \leq i \leq x$): $s[i][0]$ $s[i][1]$ $\ldots$ $s[i][N]$
\end{itemize}
That is the format of the sample data below. The sample grader does not check whether your strategy
wins the challenge; the judge does that.
